...
function Fo() {
  const t = document.querySelectorAll("iframe"), e = document.querySelector("#styleguide-previous"), n = document.querySelector("#styleguide-next"), s = (r) => {
    e && r.key === "ArrowLeft" && e.click(), n && r.key === "ArrowRight" && n.click();
  };
  t.forEach((r) => {
...
function Ro() {
  const t = document.querySelectorAll("iframe"), e = (n) => {
    n.key === "k" && (n.metaKey || n.ctrlKey) && window.dispatchEvent(new Event("styleguideOpenSearch"));
  };
  t.forEach((n) => {
...
rn && an && on && import("./icons-91zvSzCI.js").then(({ default: t }) => t(rn, an, on)).catch(console.error);
const Ss = "data-clipboard-value", ln = document.querySelectorAll(`button[${Ss}]`);
ln.length > 0 && import("./clipboard-DHll4OTv.js").then(({ default: t }) => t(ln, Ss)).catch(console.error);
